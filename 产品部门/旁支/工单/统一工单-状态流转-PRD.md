...
[已确认][改] 统一工单为**独立工单**。每次拦截、换标、重出或仓内操作各自一张工单；客户新要求**新建独立工单并填关联工单号**，不能修改已下发或已执行的旧单。验收不通过的返工退回原执行人，不另建新单。

[已确认][改] **本单完结（BR-STS-009）**：每张独立工单自行走到终态即可结案；关联在办单**不**阻塞本单结案。样板：WO-M001-01 拦截、WO-M001-02 拍照、WO-M001-03 换标各结各的，用关联工单号追溯。
