...
''',
  ...
  },
  {
    ...
    "title": "船务组 · 组流程图",
    "cls": "cw",
    "scope": "S2 合约价确认（船务主责）· 无需报价·价格表自动出价 · S3 提单 · S4 配舱 · S6 进港→离港",
    "handoffs": "H1→私卡（头程运价）· 私卡报完价回接→船务安排订舱 · 无需报价入口←改配/其他价格表渠道 · H3→关务清关 · H4→海外 · H5←关务舱单/放行只读",
    "parallel": "私卡报价轨 ∥ 无需报价·价格表自动出价 · P1：订舱配舱 ∥ 关务推送资料",
    "desc": "全流程拆组：运价列表合约价确认 → H1 下发私卡 → <b>私卡报完价后直接船务安排订舱/配舱</b>。<b>并行：无需报价分支</b>（入口=改配、其他价格表出价渠道，不含私卡）→ 仅系统按价格表自动出价 → <b>同样直接船务安排</b>（不经运价/舱位表承接）。之后提单/放行盯控 → 进港 → 离港。改配由船务发起并安排配舱/船期。",
    "refs": [
      ("../船务组-0730-费用节点提单配仓-大致方案.html", "0730 方案"),
      ("../船务组-运价管理-我司与代理订舱-MVP.html", "运价管理·确认开关"),
      ("drawio-mmd/11-L2-S2-合约价确认.mmd", "L2-S2 mmd"),
      ("drawio-mmd/12-L2-S3-提单管理.mmd", "L2-S3 mmd"),
      ("drawio-mmd/15-L2-S6-进港.mmd", "L2-S6 mmd"),
    ],
    "mermaid": r'''flowchart TB
  A["运价列表展示合约价"] --> SW{"合约价确认开关<br/>可配置开/关"}
  SW -->|需确认| OK["确认写回运价侧"]
  SW -->|仅展示| NO["保持未确认 · 仅展示"]
  OK --> H1{{"H1 → 私卡<br/>合约价报价前就绪"}}
  NO --> H1
  H1 --> H1B{{"← 私卡报完价<br/>绑运单完成"}}
  H1B --> P1A["船务安排<br/>∥ P1 订舱配舱提单"]
  ...
  classDef para fill:#E3F2FD,stroke:#1976D2,color:#0D47A1,stroke-dasharray: 5 3
  classDef endn fill:#ECEFF1,stroke:#78909C,color:#37474F
  class A,OK,NO,B,C,E,F,H,RECV,PULL,MAN,GAI,OTH,SAIL step
  class SW,TR,D,G,FT,ARR dec
  class H1,H1B,H3,H4,H5 cross
...
<body>
  <div class="page">
    ...
    <div class="map">
      ...
      <b>拆组对照：</b><br/>
      私卡 → S1（船务下发头程运价）　｜　风控 → S5 查货　｜　船务 → S2报价前确认·S3·S4配舱·S6进港→离港　｜　
